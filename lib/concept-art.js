...
// Perspective hint so the art shows the real gameplay camera, not a vista.
const TOPDOWN_GENRES = new Set([
  "social_deduction", "roguelike", "rpg", "top_down_rpg", "stealth", "moba",
  "tower_defense", "strategy", "simulation", "isometric", "sandbox", "shooter",
]);
const SIDEVIEW_GENRES = new Set([
  "platformer", "fighting", "endless_runner", "metroidvania",
]);

function perspectiveHint(archetype) {
  if (TOPDOWN_GENRES.has(archetype)) return "top-down ";
  if (SIDEVIEW_GENRES.has(archetype)) return "side-view ";
  return "";
}
...
function buildShots(prompt, visualTheme, archetype) {
  ...
  const persp = perspectiveHint(archetype); // "top-down " / "side-view " / ""
  const shots = [
    {
      caption: "overall look — the in-game map/layout, palette, and characters",
      framing:
        `${persp}in-game gameplay screenshot showing the whole play area or map, ` +
        `its rooms/zones laid out, and the characters in it as they appear during play`,
    },
  ];
  ...
  if (VISIBILITY_GENRES.has(archetype) || VISION_HINT_RE.test(p)) {
    shots.push({
      ...
      framing:
        `${persp}in-game gameplay screenshot of what the player sees during play, ` +
        `with only a small lit circle of vision around the player character and the ` +
        `rest of the map in darkness and soft shadow, walls blocking line of sight, ` +
...
// Dev/preview helper: fetch ONE shot on its own ("overview" or "player"), so
// each can be eyeballed even on the rate-limited free tier (one image per call).
// Not used by the game pipeline.
export async function generateSingleShot({ prompt, visualTheme, archetype, which = "overview" }) {
  if (!prompt || typeof prompt !== "string") return null;
  const shots = buildShots(prompt, visualTheme, archetype);
  const shot =
    which === "player"
      ? shots.find((s) => /player/i.test(s.caption)) || shots[0]
      : shots[0];
  const imgPrompt = buildImagePrompt(prompt, visualTheme, shot.framing);
  let img = await fetchPollinations(imgPrompt, "flux", FLUX_TIMEOUT_MS).catch(() => null);
  if (!img) img = await fetchPollinations(imgPrompt, "turbo", TURBO_TIMEOUT_MS).catch(() => null);
  return img ? { ...img, caption: shot.caption } : null;
}
