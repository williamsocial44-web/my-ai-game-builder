...
// Returns the most-played PUBLIC games (for the community showcase). Anyone can
// read these — the public-read RLS policy allows it. Lightweight columns only
// (no html). Returns [] if unavailable.
export async function listPublicGames(limit = 12) {
  const supabase = client();
  if (!supabase) return [];
  try {
    const { data, error } = await supabase
      .from("games")
      .select("id,title,prompt,scene,plays,visibility,created_at")
