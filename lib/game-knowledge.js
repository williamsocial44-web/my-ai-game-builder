...
export const SEED_LESSONS = [
  ...
  { category: "gameplay", lesson: "Realize limited vision from the genre without being told: social deduction (Among Us), stealth, horror, roguelikes only show a lit radius/cone around the player — the rest of the map is dark and things hide in it. A fully-lit map kills the tension.", confidence: 10 },
  { category: "visual", lesson: "Cheap, great-looking limited vision: draw the world, overlay rgba(0,0,0,0.85), then punch a soft radial-gradient hole around the player. Reveal a room when the player enters its rectangle. Add a vignette for mood.", confidence: 9 },
...
VISIBILITY & ATMOSPHERE — infer it from the genre, even when the user never says so:
- Many games are defined by what the player CANNOT see. Social deduction (Among Us), stealth, horror, roguelikes and dungeon crawlers use LIMITED VISION: a lit radius or cone around the player, with the rest of the map dimmed or hidden, and line-of-sight blocked by walls. Never render a fully-lit map for these — the tension comes from limited information and what hides in the dark.
- You must REALIZE this yourself from the request. "Among Us" → the player only clearly sees their current room and a radius around them; other rooms stay dark until entered, and a body or the impostor can lurk just out of sight. "Stealth" → guard vision cones you must avoid. "Horror"/"dungeon"/"maze" → a small light radius around the player, everything beyond it black. Do not wait to be told.
- Implement vision cheaply: draw the world, then overlay a dark layer (e.g. rgba(0,0,0,0.85)) and punch a soft-edged hole in it with a radial gradient centered on the player; optionally reveal a whole room when the player enters its rectangle. For walls blocking sight, cast a few rays from the player and darken cells behind walls.
- Add soft drop shadows under characters/objects and a subtle vignette for mood — even bright games read as more premium with gentle shadowing.
- If concept images are attached, treat any "what the player actually sees" / dark, vision-limited image as a literal spec: the running game must look like THAT during play, not like the fully-lit overview image.
